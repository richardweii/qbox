...
private:
  ...
  uint32_t ptr2bid(const char *ptr) const {
    return (ptr - memoryBase()) / BLOCK_SIZE;
  }
...
public:
  ...
  template <typename Tp> void Free(Tp *ptr) {
    char *rptr = reinterpret_cast<char *>(ptr);
    if (rptr > memoryBase() + _nbytes || rptr < memoryBase())
