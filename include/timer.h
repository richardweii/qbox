 ...
 public:
  ...
  size_t get_ns() const {
    return static_cast<size_t>(
        std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now() - start_time_)
            .count());
  }
...
static double to_sec(size_t cycles, double freq_ghz) { return (cycles / (freq_ghz * 1000000000)); }

/// Convert cycles measured by rdtsc with frequence \p freq_ghz to msec
static double to_msec(size_t cycles, double freq_ghz) { return (cycles / (freq_ghz * 1000000)); }

/// Convert cycles measured by rdtsc with frequence \p freq_ghz to usec
static double to_usec(size_t cycles, double freq_ghz) { return (cycles / (freq_ghz * 1000)); }

static size_t ms_to_cycles(double ms, double freq_ghz) { return static_cast<size_t>(ms * 1000 * 1000 * freq_ghz); }

static size_t us_to_cycles(double us, double freq_ghz) { return static_cast<size_t>(us * 1000 * freq_ghz); }

static size_t ns_to_cycles(double ns, double freq_ghz) { return static_cast<size_t>(ns * freq_ghz); }

/// Convert cycles measured by rdtsc with frequence \p freq_ghz to nsec
static double to_nsec(size_t cycles, double freq_ghz) { return (cycles / freq_ghz); }
...
class TscTimer {
 public:
  ...
  double avg_sec(double freq_ghz) const { return to_sec(avg_cycles(), freq_ghz); }

  double avg_usec(double freq_ghz) const { return to_usec(avg_cycles(), freq_ghz); }

  double avg_nsec(double freq_ghz) const { return to_nsec(avg_cycles(), freq_ghz); }
};

class RdtscTimer {
 public:
  uint64_t us() const { return to_usec(rdtsc() - start_tsc_, freq_ghz_); }
  uint64_t ms() const { return to_msec(rdtsc() - start_tsc_, freq_ghz_); }
  uint64_t sec() const { return to_sec(rdtsc() - start_tsc_, freq_ghz_); }
  static RdtscTimer &instance() {
    static RdtscTimer timer(measure_rdtsc_freq());
    return timer;
  }

 private:
  RdtscTimer(double freq_ghz) : freq_ghz_(freq_ghz), start_tsc_(rdtsc()) {}
  uint64_t start_tsc_;
  double freq_ghz_;
};