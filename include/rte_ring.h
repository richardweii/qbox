...
class RteRing {
 public:
  ...
  Status enqueue(uint32_t n, T **obj_table, uint32_t *res_n = nullptr, bool fixed = false) {
    if (mode & RteRingMode::MP)
  ...
  Status dequeue(uint32_t n, T **obj_table, uint32_t *res_n = nullptr, bool fixed = false) {
    if (mode & RteRingMode::MC)
 ...
 private:
  ...
  Status enqueue_mp(T *obj_table) {
    ...
    do {
      ...
      prod_next = prod_head + 1;
      if (__atomic_compare_exchange_n(&prod.prod_head_, &prod_head, prod_next, false, __ATOMIC_ACQ_REL,
                                      __ATOMIC_RELAXED)) {
        break;
      }
    } while (true);
  ...
  Status enqueue_sp_bulk(uint32_t n, T **obj_table, uint32_t *res_n, bool fixed) {
    uint32_t prod_head, cons_tail;
    uint32_t prod_next, free_entries;
  ...
  Status enqueue_mp_bulk(uint32_t n, T **obj_table, uint32_t *res_n, bool fixed) {
    uint32_t prod_head, cons_tail;
    uint32_t prod_next, free_entries;
    uint32_t max_n = n;
    unsigned rep = 0;
  ...
  Status dequeue_mc(T **obj_table) {
    ...
    do {
      ...
      if (__atomic_compare_exchange_n(&cons.cons_head_, &cons_head, cons_next, false, __ATOMIC_ACQ_REL,
                                      __ATOMIC_RELAXED)) {
        break;
      }
    } while (true);
  ...
  Status dequeue_sc_bulk(uint32_t n, T **obj_table, uint32_t *res_n, bool fixed) {
    uint32_t cons_head, prod_tail;
    uint32_t cons_next, entries;
  ...
  Status dequeue_mc_bulk(uint32_t n, T **obj_table, uint32_t *res_n, bool fixed) {
    uint32_t cons_head, prod_tail;
    uint32_t cons_next, entries;
    uint32_t max_n = n;
    unsigned rep = 0;
